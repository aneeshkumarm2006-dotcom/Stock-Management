...
export function computeWarnings(
  ...
  switch (type) {
    case "Property": {
      ...
      if (strBlank(entity.propertySubType))
        out.push(mk("MISSING_PROPERTY_SUBTYPE"));
      ...
      if (fkBlank(entity.operatingAccountId))
        out.push(mk("NO_OPERATING_ACCOUNT"));
      const owners = (entity.rentalOwners ?? []) as Array<
      ...
      if (owners.length > 0) {
        const sum = owners.reduce(
          (a, r) =>
            a +
            (Number.isFinite(r.ownershipPct) ? (r.ownershipPct as number) : 0),
          0,
        );
      ...
      const mfa = entity.managementFeeAgreement as
        | Record<string, unknown>
        | null
        | undefined;
      if (mfa && mfa.active) {
        const hasPct = mfa.feePercent != null && Number(mfa.feePercent) > 0;
        const hasFlat =
    ...
    case "WorkOrder": {
      ...
      const charge = entity.chargeWorkTo as
        | Record<string, unknown>
        | null
        | undefined;
    ...
    case "DraftLease": {
      ...
      const primary = entity.primaryRent as
        | Record<string, unknown>
        | null
        | undefined;
