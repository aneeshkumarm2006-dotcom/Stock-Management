// /properties/rentals/rental-owners — list view.
// Yellow "Agreement ends on" chip appears when daysUntilAgreementEnds <= 90.
"use client";
...
export default function RentalOwnersPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    const r = await fetch(`/api/pm/rental-owners?${params.toString()}`);
    if (r.ok) setRows((await r.json()) as OwnerRow[]);
    setLoading(false);
  }, [filter, search]);
      ...
      <Card>
        <CardHeader>
          ...
          <Button size="sm" onClick={() => setModalOpen(true)}>
            <Plus className="h-3.5 w-3.5" /> Add rental owner
          </Button>
        </CardHeader>
              ...
              {visible.map((o) => (
                ...
                >
                  <td className="py-2 text-fg">
                    <Link
                      href={`/properties/rentals/rental-owners/${o.id}`}
                      className="font-medium hover:underline"
                    >
                      {o.displayName}
                    </Link>
                  </td>
                  <td className="text-fg-muted">{o.primaryEmail || "—"}</td>
                  <td className="text-fg-muted">
                    <AgreementChip
                      endDate={o.managementAgreementEndDate}
                      days={o.daysUntilAgreementEnds}
                    />
                  </td>
                  <td className="text-right">{!o.active && "Inactive"}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function AddRentalOwnerModal({
  ...
  async function save() {
    ...
    setSaving(true);
    const res = await fetch("/api/pm/rental-owners", {
      method: "POST",
    ...
    toast({ title: "Rental owner added", variant: "success" });
    reset();
    onClose();
    await onSaved();
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg">
        <DialogHeader title="Add rental owner" onClose={onClose} />
        <div className="space-y-3">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="space-y-1">
              <Label htmlFor="ro-first">First name *</Label>
              <Input
                id="ro-first"
            ...
            <div className="space-y-1">
              <Label htmlFor="ro-last">Last name *</Label>
              <Input
                id="ro-last"
                value={form.lastName}
                onChange={(e) =>
          ...
          <div className="space-y-1">
            <Label htmlFor="ro-email">Primary email</Label>
            <Input
              id="ro-email"
              type="email"
          ...
          <p className="text-xs text-fg-muted">
            Phone numbers, address, agreement dates, and 1099 settings can be
            edited from the detail page after creation.
          </p>
        </div>
        <DialogFooter>
