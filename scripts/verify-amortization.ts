...
function runPureChecks(): void {
  ...
  check(`${combos} combinations amortize to exactly the original principal`, sumOk);
  check(
  ...
  check('interest falls and principal rises while a rate is charged', monotonicOk);
  check('amortizationAt(t, k) matches amortizationSchedule(t)[k-1]', atMatchesScheduleOk);
  ...
  check('a 0% loan has a 0 periodic rate', periodicRate({
...
async function runLiveChecks(): Promise<void> {
  ...
  const { RecurringTransaction } = await import(
    '../lib/db/models/pm/RecurringTransaction'
  );
  ...
  for (const rule of rules) {
    ...
    if (m.principalAccountId) {
      const glPrincipal = jes.reduce(
        (s, j) =>
          ...
          j.lines
            .filter(
              (l) => String(l.accountId) === String(m.principalAccountId),
            )
    ...
    if (m.statementBalanceCents != null && m.statementDate) {
      ...
      check(
        ...
        delta <= 500,
        `ours ${row.closingBalanceCents}c vs statement ${m.statementBalanceCents}c (Δ ${delta}c)`,
      );
    } else {
      console.log(
